 ...
 * store.
 *
 * Note: This is the backing store object for TJSGameSettings you don't need this if you are using TJSGameSettings
 *
 ...
 */
export class TJSGameSettingStores
{
   /** @type {string} */
   #namespace;
   ...
   /**
    * Creates the TJSGameSettingStores instance.
    *
   ...
   #getStore(key, initialValue)
   {
      ...
      if (store === void 0)
      {
         store = TJSGameSettingStores.#createStore(initialValue);
   ...
   getReadableStore(key)
   {
      ...
      {
         console.warn(`TJSGameSettingStores - getReadableStore: '${key}' is not a registered setting.`);
         return;
      }
   ...
   getWritableStore(key)
   {
      if (!this.#stores.has(key))
      {
         console.warn(`TJSGameSettingStores - getWritableStore: '${key}' is not a registered setting.`);
         return;
      }
   ...
   /**
    * Registers a setting with TJSGameSettingStores and Foundry core.
    *
    ...
    *
    * This allows the custom store in the `set` implementation to mainly only trigger the TJSGameSettingStores subscriber
   ...
   register(setting, coreConfig = true)
   {
      if (!isObject(setting))
      {
         throw new TypeError(`TJSGameSettingStores - register: setting is not an object.`);
      }

      if (!isObject(setting.options))
      {
         throw new TypeError(`TJSGameSettingStores - register: 'setting.options' attribute is not an object.`);
      }

      if (typeof coreConfig !== 'boolean')
      {
         throw new TypeError(`TJSGameSettingStores - register: 'coreConfig' is not an boolean.`);
      }

      if (setting.store !== void 0 && !isWritableStore(setting.store))
      {
         throw new TypeError(
          `TJSGameSettingStores - register: 'setting.store' attribute is not a writable store.`);
      }
      ...
      {
         throw new TypeError(`TJSGameSettingStores - register: 'namespace' attribute is not a string.`);
      }

      if (typeof key !== 'string')
      {
         throw new TypeError(`TJSGameSettingStores - register: 'key' attribute is not a string.`);
      }

      if (folder !== void 0 && typeof folder !== 'string')
      {
         throw new TypeError(`TJSGameSettingStores - register: 'folder' attribute is not a string.`);
      }
    ...
    *
    * Please refer to the note in {@link TJSGameSettingStores.register} about the returned object of store subscriber handler
    * functions.
    *
   ...
   registerAll(settings, coreConfig)
   {
      ...
      if (!isIterable(settings)) { throw new TypeError(`TJSGameSettingStores - registerAll: settings is not iterable.`); }

      for (const entry of settings)
      {
         if (!isObject(entry))
         {
            throw new TypeError(`TJSGameSettingStores - registerAll: entry in settings is not an object.`);
         }

         if (typeof entry.namespace !== 'string')
         {
            throw new TypeError(`TJSGameSettingStores - registerAll: entry in settings missing 'namespace' attribute.`);
         }

         if (typeof entry.key !== 'string')
         {
            throw new TypeError(`TJSGameSettingStores - registerAll: entry in settings missing 'key' attribute.`);
         }

         if (!isObject(entry.options))
         {
            throw new TypeError(`TJSGameSettingStores - registerAll: entry in settings missing 'options' attribute.`);
         }
