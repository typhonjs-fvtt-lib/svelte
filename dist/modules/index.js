import { SvelteComponent, init, safe_not_equal, append_styles, empty, insert, group_outros, transition_out, check_outros, transition_in, detach, element, attr, noop, create_component, mount_component, get_spread_update, get_spread_object, destroy_component, destroy_each, assign, text, append, listen, set_data, space, binding_callbacks, set_style, add_render_callback, create_bidirectional_transition, current_component } from 'svelte/internal';
...
function add_css(target) {
	append_styles(target, "svelte-18rvdrh", ".menu.svelte-18rvdrh{position:fixed;width:fit-content;font-family:Signika sans-serif;font-size:14px;box-shadow:0 0 10px #000;height:max-content;min-width:150px;max-width:360px;background:#23221d;border:1px solid #000;border-radius:5px;color:#EEE;z-index:10000}");
}
...
function create_fragment(ctx) {
	...
	return {
		c() {
			...
			attr(div, "class", "menu svelte-18rvdrh");
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = listen(document.body, "click", /*onClose*/ ctx[4]);
				mounted = true;
			}
		},
		p(ctx, [dirty]) {
		...
		i(local) {
			...
			add_render_callback(() => {
				if (!div_transition) div_transition = create_bidirectional_transition(div, slideFade, { duration: 400 }, true);
		...
		o(local) {
			if (!div_transition) div_transition = create_bidirectional_transition(div, slideFade, { duration: 400 }, false);
...
function instance($$self, $$props, $$invalidate) {
	...
	return [x, y, items, menuEl, onClose, div_binding];
}

class TJSContextMenu extends SvelteComponent {
	constructor(options) {
		super();
		init(this, options, instance, create_fragment, safe_not_equal, { x: 0, y: 1, items: 2 }, add_css);
	}
}
